...
> 2026-04-26 更新：代码已改为支持从 `input/02_03_pitchbook_sample_100_shared.xlsx` 直接生成 `BoardCase`。新版 `BoardCase` 删除了 `runway_months`、`burn_multiple`、`tech_debt_risk` 等无法从表格直接或确定性计算得到的主观字段；真实结果字段只保存在 `notes.labels` 中用于后续评测，不进入 Agent 可见上下文。本文档中部分早期 MVP 字段说明保留作历史背景，实际代码以 `boardroom_sim/models.py` 和 `boardroom_sim/pitchbook.py` 为准。
