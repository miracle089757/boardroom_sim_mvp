...
命令行参数 `--config` 可以指定其它 TOML 文件。`--bargaining-rounds` 会覆盖最大讨论轮数，`--history-limit` 会覆盖历史窗口。
...
- 当前董事会过程约束，包括 board archetype、board_archetype_description、meeting_phase、解释后的 L5 culture、discussion protocol、challenge_required、memory_scope 和当前有效权重

LLM 必须返回：

- 一段更接近真实会议的董事会发言 `message`
- 会议行为元数据，例如 `boardroom_act`、`direct_response_to`、`questions_raised`、`evidence_anchors`、`alternative_options` 和 `role_commitment`
- 更新后的完整预测 JSON
...
当前 `board.archetype` 还会决定每轮会议阶段。流程不再固定三轮，而是由 `discussion.min_rounds`、`discussion.max_rounds` 和每轮后的终止条件共同控制。

默认设置是至少 2 轮、最多 6 轮。每轮结束后，`BoardProcessController.should_continue_discussion()` 会根据以下信号判断是否继续：
...
具体阶段会随 `aunt`、`barbarian`、`clan`、`value_creating` 变化。

### 8. 流程终止与最终聚合

讨论结束可能来自两类原因：

- 达到 `discussion.max_rounds`；
- 已达到最小轮数，角色预测稳定，没有新的开放问题，且共识分数达到 `stop_when_consensus_score_gte`。

每轮后的继续/结束理由会写入 trace 的 `process_decision_after_round_N`，并在可读 Markdown 报告中展示。

讨论结束后，模拟器按“基础权重 × archetype profile 中的角色权重乘数 × 可选配置覆盖”得到的有效权重聚合所有角色最新预测，生成最终 `SimulationResult`。
